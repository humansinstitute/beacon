...
import { saveMessage, lastMessages, getHistoryByThreadId } from '../../services/store/conversationStore.js';
...
async function researchAnswer(message, npub, threadId) {
    try {
        ...
        try {
            ...
            console.log('Bot message saved to store. ID:', savedBotMsg.id, 'Thread ID:', savedBotMsg.threadId);
        } catch (botSaveError) {
            console.error('Error saving bot message to conversation store:', botSaveError);
        }
        ...
        const errorSent = await client.sendMessage(message.from, 'Sorry, there was an error processing your request.');
        // Save bot's outgoing error message to conversation store
        try {
            const savedErrMsg = await saveMessage(errorSent, npub);
            console.log('Bot error message saved to store. ID:', savedErrMsg.id, 'Thread ID:', savedErrMsg.threadId);
        } catch (botSaveError) {
            console.error('Error saving bot error message to conversation store:', botSaveError);
        }
        return false;
    }
}
...
client.on('message_create', async (message) => {
    ...
    try {
        ...
        if (err.response && err.response.status === 404) {
            ...
            const identitySent = await client.sendMessage(message.from, "Hi I'm curently running an Avalon AI and I don't recognise your user ID!");
            // Save bot identity message to store
            try {
                const savedIdentityMsg = await saveMessage(identitySent, npub);
                console.log('Bot identity message saved to store. ID:', savedIdentityMsg.id, 'Thread ID:', savedIdentityMsg.threadId);
            } catch (botSaveError) {
                console.error('Error saving bot identity message to conversation store:', botSaveError);
            }
            return;
        } else {
    ...
    try {
        ...
        console.log('Message saved to store. ID:', storedMessage.id, 'Thread ID:', storedMessage.threadId);
    } catch (error) {
    ...
    try {
        ...
        if (!billingDoc || billingDoc.budget < 0.10) {
            const budgetSent = await message.reply("This app is out of budget, please contact Pete!");
            // Save budget insufficient message to store
            try {
                const savedBudgetMsg = await saveMessage(budgetSent, npub);
                console.log('Bot budget insufficient message saved to store. ID:', savedBudgetMsg.id, 'Thread ID:', savedBudgetMsg.threadId);
            } catch (botSaveError) {
                console.error('Error saving bot budget message to conversation store:', botSaveError);
            }
        } else {
            ...
            switch (intentObject.message.intent) {
                ...
                case 'event':
                    ...
                    await researchAnswer(message, npub || message.from, storedMessage.threadId);
                    // Additional budget decrement for researchAnswer.
                    billingDoc.budget -= 0.05;
                    await billingDoc.save();
                    break;
                case 'conversation':
                    const convSent = await message.reply(intentObject.message.quickResponse);
                    // Save conversation quickResponse message to store
                    try {
                        const savedConvMsg = await saveMessage(convSent, npub);
                        console.log('Bot conversation response saved to store. ID:', savedConvMsg.id, 'Thread ID:', savedConvMsg.threadId);
                    } catch (botSaveError) {
                        console.error('Error saving bot conversation response to conversation store:', botSaveError);
                    }
                ...
                case 'post':
                    try {
                        ...
                        try {
                            ...
                            console.log('Bot post success message saved to store. ID:', savedPostMsg.id, 'Thread ID:', savedPostMsg.threadId);
                        } catch (botSaveError) {
                            console.error('Error saving bot post success message to conversation store:', botSaveError);
                        }

                    } catch (err) {
                        console.error('Error in "post" intent processing:', err.response ? err.response.data : err.message);
                        const postErrSent = await client.sendMessage(message.from, `Sorry, I wasn't able to send your note to Nostr. Please try again later.`);
                        // Save bot post error message to store
                        try {
                            const savedPostErrMsg = await saveMessage(postErrSent, npub);
                            console.log('Bot post error message saved to store. ID:', savedPostErrMsg.id, 'Thread ID:', savedPostErrMsg.threadId);
                        } catch (botSaveError) {
                            console.error('Error saving bot post error message to conversation store:', botSaveError);
                        }
                    }
                    break;
                ...
                default:
                    ...
                    if (intentObject.message.quickResponse) {
                        const defaultSent = await message.reply(intentObject.message.quickResponse);
                        // Save default quickResponse message to store
                        try {
                            const savedDefaultMsg = await saveMessage(defaultSent, npub);
                            console.log('Bot default quickResponse message saved to store. ID:', savedDefaultMsg.id, 'Thread ID:', savedDefaultMsg.threadId);
                        } catch (botSaveError) {
                            console.error('Error saving bot default quickResponse message to conversation store:', botSaveError);
                        }
                    } else {
                        // Fallback if quickResponse is also missing for some reason
                        const fallbackSent = await message.reply("I'm not sure how to handle that request right now.");
                        // Save default fallback message to store
                        try {
                            const savedFallbackMsg = await saveMessage(fallbackSent, npub);
                            console.log('Bot default fallback message saved to store. ID:', savedFallbackMsg.id, 'Thread ID:', savedFallbackMsg.threadId);
                        } catch (botSaveError) {
                            console.error('Error saving bot default fallback message to conversation store:', botSaveError);
                        }
        ...
        const catchSent = await message.reply('Sorry, there was an error processing your request.');
        // Save bot catch error message to store
        try {
            const savedCatchMsg = await saveMessage(catchSent, npub);
            console.log('Bot catch error message saved to store. ID:', savedCatchMsg.id, 'Thread ID:', savedCatchMsg.threadId);
        } catch (botSaveError) {
            console.error('Error saving bot catch error message to conversation store:', botSaveError);
        }
    }
});
