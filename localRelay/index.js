...
const server = http.createServer((req, res) => {
    if (req.headers.accept && req.headers.accept.includes('application/nostr+json')) {
        const relayInfo = {
...
function eventMatchesFilters(event, filters) {
    return filters.some(filter => {
        if (filter.ids && !filter.ids.some(idPref => event.id.startsWith(idPref))) return false;
...
function handleEventPublication(event, socket) {
    ...
    try {
        if (getEventHash(event) !== event.id || !verifySignature(event)) {
            socket.send(JSON.stringify(["OK", event.id || "", false, "invalid event"]));
            return;
        }
    } catch (e) {
        socket.send(JSON.stringify(["OK", event.id || "", false, "verification error"]));
        return;
    }
    ...
    if (!(event.kind >= 20000 && event.kind < 30000)) {
        ...
        if (event.kind === 0 || event.kind === 3 || (event.kind >= 10000 && event.kind < 20000)) {
            db.prepare('DELETE FROM events WHERE pubkey = ? AND kind = ?')
                .run(event.pubkey, event.kind);
        } else if (event.kind >= 30000 && event.kind < 40000) {
            const dTag = event.tags.find(t => t[0] === "d");
            if (dTag) {
                const pattern = `%["d","${dTag[1]}"%`;
                db.prepare('DELETE FROM events WHERE pubkey = ? AND kind = ? AND tags LIKE ?')
